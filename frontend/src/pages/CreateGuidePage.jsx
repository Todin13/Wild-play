...
import { useAddGuide } from "@/hooks/GuideHooks";
...
const CreateGuidePage = () => {
  ...
  const [guideData, setGuideData] = useState({
    title: "",
    duration: "",
    locations: [{ name: "", section: "", info: "", lat: 0, lon: 0 }],
    notes: [],
  });
  ...
  const handleLocationChange = (e, index) => {
    const { name, value } = e.target;
    const updatedLocations = [...guideData.locations];
    updatedLocations[index][name] = value;
    setGuideData((prev) => ({ ...prev, locations: updatedLocations }));
  };

  const addLocation = () => {
    setGuideData((prev) => ({
      ...prev,
      locations: [...prev.locations, { name: "", lat: 0, lon: 0 }],
    }));
  };

  const removeLocation = (index) => {
    if (guideData.locations.length > 1) {
      const updatedLocations = [...guideData.locations];
    ...
    try {
      ...
      alert(
        creatingGuideError?.message ||
          error?.message ||
          "Something went wrong while creating the guide."
      );
    }
  };
  const scrollToMap = () => {
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <div className="text-center">
                ...
                >
                  {creatingGuideLoading ? "Creating..." : "Create Guide"}
                </button>
              </div>
            </form>
          </div>
