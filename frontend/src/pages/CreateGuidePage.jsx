...
const CreateGuidePage = () => {
  const { addGuide, creatingGuideLoading, creatingGuideError } = useAddGuide();
  const { update, updatingGuideLoading, updatingGuideError } = useUpdateGuide();

  const location = useLocation();
  const { guide } = location.state || {};
  const isEditing = guide && Object.keys(guide).length > 0;

  const { goToGuideDetail } = useNavigationHooks();
  ...
  const [guideData, setGuideData] = useState({
    title: isEditing ? guide.title : "",
    duration: isEditing ? guide.duration : "",
    locations: isEditing
      ? [
          ...guide.locations,
  ...
  const handleLocationChange = (e, index) => {
    ...
    setGuideData((prev) => ({ ...prev, locations: updated }));
  };
  ...
  const addNote = () => {
    setGuideData((prev) => ({ ...prev, notes: [...prev.notes, ""] }));
  };

  const handleNoteChange = (e, index) => {
    const updated = [...guideData.notes];
    updated[index] = e.target.value;
    setGuideData((prev) => ({ ...prev, notes: updated }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditing) {
        const response = await update(guide._id, guideData);
        alert("Guide updated successfully!");
        goToGuideDetail(response.guide);
      } else {
      ...
      const message =
        creatingGuideError?.message ||
        updatingGuideError?.message ||
        error?.message ||
        "Something went wrong.";
      console.log(creatingGuideError?.message);
      console.log(updatingGuideError?.message);
    ...
    <MainLayout>
      <div className="w-full px-4 lg:px-12 py-16 mx-auto min-w-[95vw]">
        {/* <h2 className="text-3xl font-bold text-center text-voga-title lg:px-12">
            {isEditing ? "Update Guide" : "Create New Guide"}
          </h2> */}
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="w-full lg:w-[45%] bg-white p-8 rounded-3xl shadow-xl border border-gray-300 space-y-8 overflow-y-auto">
            <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <div className="space-y-6">
                ...
                <DndContext
                  ...
                  onDragStart={({ active }) => {
                    ...
                    setActiveLocation(guideData.locations[index]);
                  }}
                  onDragCancel={() => {
                    setActiveId(null);
                ...
                >
                  <SortableContext
                    items={guideData.locations.map((_, index) =>
                      index.toString()
                    )}
                    strategy={verticalListSortingStrategy}
                  >
                    {Object.entries(
                      guideData.locations.reduce((groups, loc, idx) => {
                      ...
                      <SectionGroup
                        ...
                        handleLocationChange={handleLocationChange}
                        totalLocations={guideData.locations.length}
              ...
              <div className="space-y-4">
                ...
                {guideData.notes.map((note, index) => (
              ...
              <div className="text-center">
                <button
                  type="submit"
                  disabled={creatingGuideLoading || updatingGuideLoading}
                ...
                >
                  {creatingGuideLoading || updatingGuideLoading
                    ? isEditing
                      ? "Updating..."
                      : "Creating..."
                    : isEditing
          ...
          >
            ...
            <MapContainer center={[20, 0]} zoom={2} className="w-full h-full">
              ...
              />
              {guideData.locations.map((loc, idx) =>
                loc.lat || loc.lon ? (
                  <Marker
                    key={idx}
