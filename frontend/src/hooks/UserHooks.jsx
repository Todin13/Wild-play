...
Link for react and the function linked to the backend for the user
Author: Xiang Yu Oon
...
export const UserProvider = ({ children }) => {
  ...
  return (
    <UserContext.Provider
      value={{ user, loading, error, success, handleLogin }}
    >
      {children}
    </UserContext.Provider>
  );
...
export const useRegister = () => {
  ...
  useEffect(() => {
    const loadCountryCodes = async () => {
      try {
        ...
        if (codes.length > 0) {
          setSelectedCode(
            codes.find((c) => c.code === "+353")?.code || codes[0].code
          );
        }
      } catch (err) {
  ...
  const handleRegister = async (userData) => {
    ...
    try {
      ...
      if (
        result.message ===
        "User saved successfully! Please log in for further possibility"
      ) {
  ...
  return {
    ...
    success,
    setError,
  }; // Set default to 353 or first code if not found
};
  ...
  useEffect(() => {
    const loadProfile = async () => {
      ...
      try {
        const { ok, data } = await fetchUserProfile(); // Call API
        if (ok) {
          // Set all the variable to what the code get from API
          setFirstName(data.detail.firstName || "");
          setLastName(data.detail.lastName || "");
          setEmail(data.detail.email || "");
          if (
            Array.isArray(data.detail.phone) &&
            data.detail.phone.length === 2
          ) {
...
export function useUserTable() {
  ...
  const handleBirthdateChecked = (e) => {
    // Handle birthdate field, if checked it will included as query, vise versa
