...
const VanCarousel = ({ vans = [] }) => {
  ...
  return (
    <div ref={viewportRef} className="hidden lg:flex justify-center items-center gap-6 h-full">
      <div className="flex-1 transform scale-75 opacity-50 transition-all duration-300 max-h-full">
        {vans[prev] && <VanCard van={vans[prev]} />}
      </div>

      <div className="flex-[2] transform scale-110 opacity-100 transition-all duration-300 relative z-10 max-h-full">
        {vans[index] && <VanCard van={vans[index]} large />}
      </div>
