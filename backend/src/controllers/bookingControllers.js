...
const setBooking = async (req, res) => {
  // validate and save new booking data in database
  try {
...
const editBooking = async (req, res) => {
  try {
    ...
    const isUserAuthorized =
      req.user.id.toString() === booking.user_id.toString();
...
const changeBookingStatus = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error occured while changing booking status", error }); // if error return 500
  }
};
...
const deleteBooking = async (req, res) => {
  try {
    ...
    const isUserAuthorized =
      user_type === "ADMIN" || booking.user_id.toString() === id.toString();
    console.log(
      "Is auth:",
      isUserAuthorized,
      "booking.user_id:",
    ...
    res
      .status(500)
      .json({ message: "Error occurred while deleting a booking", error });
  }
};
...
const getAvailableCampers = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Error retrieving available campers", error });
  }
};
