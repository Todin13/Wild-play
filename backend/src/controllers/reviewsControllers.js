...
exports.createVanReview = async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ message: "Van review added successfully", review: newReview });
  } catch (error) {
    res
      .status(500)
      .json({ message: "Error creating van review", error: error.message });
  }
};
...
exports.getVanReviews = async (req, res) => {
  try {
    ...
    const reviews = await VanReview.find(filter).populate(
      "user_id",
      "username"
    );
    ...
    res
      .status(500)
      .json({ message: "Error fetching van reviews", error: error.message });
  }
};
...
exports.getGuideReviews = async (req, res) => {
  try {
    ...
    const reviews = await GuideReview.find(filter).populate(
      "user_id",
      "username"
    );
    res.status(200).json(reviews);
    ...
    res
      .status(500)
      .json({ message: "Error fetching guide reviews", error: error.message });
  }
};
